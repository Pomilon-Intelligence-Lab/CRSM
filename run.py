...
def main():
    ...
    parser.add_argument('--task', type=str, default='lm', choices=['lm', 'distill', 'arc', 'reasoning'], help="Task to run")
    parser.add_argument('--checkpoint', type=str, help="Path to pretrained SFT checkpoint for reasoning task")
    ...
    # 2. Instantiate Model
    if args.task in ['lm', 'arc', 'reasoning']:
        ...
        if args.checkpoint:
            logger.info(f"Loading SFT checkpoint: {args.checkpoint}")
    ...
    if args.task == 'reasoning':
        logger.info(">>> STARTING RL REASONING LOOP (GRPO) <<<")
        trainer = RLTrainer(model, optimizer, config)
    else:
